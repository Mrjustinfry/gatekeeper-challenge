...
/*
function isUser(user, pass, name) {
 return name === user.userName && pass === user.password
}
function gateKeeper(req, res, next) {
   const parsedInfo = {};
  if(!req.headers['x-username-and-password'] ){
    next();
    return;
  }
  
  req.headers['x-username-and-password']
      .split('&')
      .map(data => data.split('='))
      .forEach(values => {
       parsedInfo[values[0]] = values[1];
   });
  
  const user = USERS.find(usr => isUser(usr, parsedInfo.pass, parsedInfo.user));
  if(user) {
    req.user = user;
  }
  next();
